 ...
 * 饼状图表
 */
public class PieChart_active extends View {
    /**
     * 视图的宽和高
     */
    ...
    public PieChart_active(Context context) {
        super(context);
        init(context);
    }

    public PieChart_active(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public PieChart_active(Context context, AttributeSet attrs, int defStyleAttr) {
...
//                canvas.drawText(resToRound+"%",pxt+30,pyt,mTextPaint);
    ...
    private void check(Canvas canvas,boolean flag,int i,double resToRound,float pxt,float pyt){
        switch (i){
            case 0:
                if (flag==true) {
                    canvas.drawText("高度活跃用户", pxt - mTextPaint.measureText(resToRound + "%") - 30, pyt, mTextPaint);
                }else{
                    canvas.drawText("高度活跃用户", pxt + 30, pyt, mTextPaint);
                }
                break;
            case 1:
                if (flag==true) {
