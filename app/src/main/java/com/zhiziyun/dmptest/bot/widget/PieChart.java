...
//            canvas.drawArc(mRectF,startAngle,sweepAngle,true,mPaint);
            ...
            if (startAngle % 360.0 >= 90.0 && startAngle % 360.0 <= 270.0) {//2 3 象限
                canvas.drawLine(pxt, pyt, pxt - 0, pyt, mLinePaint);
                canvas.drawText(resToRound + "%", pxt - mTextPaint.measureText(resToRound + "%") - 0, pyt, mTextPaint);
//                check(canvas,true,i,resToRound,pxt,pyt);
            } else {//1 4象限
                canvas.drawLine(pxt, pyt, pxt + 0, pyt, mLinePaint);
                canvas.drawText(resToRound+"%",pxt+0,pyt,mTextPaint);
