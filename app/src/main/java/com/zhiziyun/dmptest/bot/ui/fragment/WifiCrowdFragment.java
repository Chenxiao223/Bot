...
public class WifiCrowdFragment extends Fragment implements View.OnClickListener {
    public static WifiCrowdFragment wifiCrowdFragment;
    ...
    private final int FLAG = 123;

    @Nullable
    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_wifi_crowd, container, false);
    }
    ...
    private void initView() {
        ...
        adapter.setDetails(new CrowdAdapter.Details() {
            ...
            public void setDetails(String id, int position) {
                Intent it = new Intent(getActivity(), AddWifiCorwdActivity.class);
            ...
            public void run() {
                try {
                    ...
                    final Request request = new Request.Builder()
                            .url(BaseUrl.BaseWang + "deviceSegmentInfo/toggleStatus.action")
                            .post(body)
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    RequestBody body = RequestBody.create(mediaType, url);
                    final Request request = new Request.Builder()
                            .url(BaseUrl.BaseTest + "wifiSegmentInfo/insertTask")
                            .post(body)
                            .addHeader("content-type", "application/x-www-form-urlencoded")
                            .build();
                    ...
                    try {
                        ...
                        if (cl.getTotal() == 0) {
                            ...
                            for (int i = 0; i < cl.getRows().size(); i++) {
                                 ...
                                 * EndTime返回不为空，但若当前日期>=EndTime，说明已完成，当Count返回为0时，显示已完成
                                ...
                                if (cl.getRows().get(i).isHasTranForPhone()) {
                                    if (TextUtils.isEmpty((String) cl.getRows().get(i).getTaskStatus())) {
                                        //如果返回的不是数字而是空，就传3进去让开关隐藏
                                        hm_crowd.put("content4", 3);
                                    } else {
