...
public class RealtyActivity extends BaseActivity implements View.OnClickListener {
    ...
    private EditText et_text2, et_text1, edit_url1, et_text3, et_text4, edit_url2, edit_url3;
    ...
    private void initView() {
        ...
        et_text4 = (EditText) findViewById(R.id.et_text4);
            ...
            case R.id.btn_commit:
                if (ClickUtils.isFastClick()) {
                    mHandler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            et_text1.setFocusable(false);//为了避免光标也在截图中，点击提交后去掉光标
                            et_text2.setFocusable(false);
                            if (flag == 1) {//区分模板
                                if (TextUtils.isEmpty(edit_url1.getText().toString())) {
                                    ToastUtils.showShort(RealtyActivity.this, "请将URL填写完整");
                                } else {
                                    //加载动画
                                    dialog.show();
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 1:
                    ToastUtils.showShort(RealtyActivity.this, "图片上传成功");
                    dialog.dismiss();
                    finish();
                    break;
                case 2:
                    ToastUtils.showShort(RealtyActivity.this, "图片上传失败");
                    dialog.dismiss();
                    break;
            }
        }
