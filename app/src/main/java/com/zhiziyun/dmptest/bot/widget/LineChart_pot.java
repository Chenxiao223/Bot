...
import com.zhiziyun.dmptest.bot.util.DensityUtil;
 ...
 * 时段fragment专用
 */
public class LineChart_pot extends View {
    ...
    //左边Y轴的单位
    private String leftAxisUnit = "";//这里填写单位
    /**
     * 两个点之间的距离
     */
    private int space;
    ...
    public LineChart_pot(Context context) {
        super(context);
        init(context);
    }

    public LineChart_pot(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public LineChart_pot(Context context, AttributeSet attrs, int defStyleAttr) {
...
//                canvas.drawLine(xStartIndex, startY, mTotalWidth - leftMargin*2, startY, axisPaint);
    ...
    private void getItemsWidth() {
        space = DensityUtil.dip2px(getContext(), 63);//这里设置列之间的宽度
